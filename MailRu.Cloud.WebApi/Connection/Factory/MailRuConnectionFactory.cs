...
{
    public sealed class MailRuConnectionFactory : IMailRuConnectionFactory
    {
            ...
            )
        {
            ...
            var result = new MailRuConnection(
                req
                );
            ...
            )
        {
            ...
            string reqString = string.Format("Login={0}&Domain={1}&Password={2}&FailPage&page=https://cloud.mail.ru/?new_auth_form=1", login, ConstSettings.Domain, HttpUtility.UrlEncode(password));
            byte[] requestData = Encoding.UTF8.GetBytes(reqString);
            var request = (HttpWebRequest)WebRequest.Create(string.Format("{0}/cgi-bin/auth?from=splash", ConstSettings.AuthDomen));
            //string reqString = string.Format("Login={0}&Domain={1}&Password={2}", login, ConstSettings.Domain, HttpUtility.UrlEncode(password));
            //byte[] requestData = Encoding.UTF8.GetBytes(reqString);
